...
public abstract class GsonUtils {
    ...
    private static final Type TYPE_OF_STRING_LIST = new TypeToken<ArrayList<String>>() {
    }.getType();
    private static final Type TYPE_OF_STRING_SET = new TypeToken<HashSet<String>>() {
    }.getType();
    private static final Type TYPE_OF_LONG_SET = new TypeToken<HashSet<Long>>() {
    }.getType();
    private static final Type TYPE_OF_INTEGER_SET = new TypeToken<HashSet<Integer>>() {
    }.getType();
     ...
     *
     * @param obj JSON字符串或者bean对象
     * @return 漂亮紧凑的JSON字符串
     */
    public static String toPrettyJson(Object obj) {
        if (obj instanceof String) {
            return pretty.toJson(jsonParser.parse((String) obj));
        }
        return pretty.toJson(obj);
    }

    /**
     * 解析JSON字符串或者bean对象为JSON对象
     *
     * @param obj JSON字符串或者bean对象
     * @return JSON对象
     * @throws JsonSyntaxException 转换异常,json不合法或者目标对象类型不合法
     */
    public static JsonObject toJsonObject(Object obj) throws JsonSyntaxException {
        if (obj instanceof String) {
    ...
    /**
     * 解析JSON字符串或者bean对象为JSON对象数组
     *
     * @param obj JSON字符串或者bean对象
     * @return JSON对象数组
     * @throws JsonSyntaxException 转换异常,json不合法或者目标对象类型不合法
     */
    public static JsonArray toJsonArray(Object obj) throws JsonSyntaxException {
        if (obj instanceof String) {
            return jsonParser.parse((String) obj).getAsJsonArray();
        }
        return gson.toJsonTree(obj).getAsJsonArray();
    }

    /**
     * 解析JSON字符串为String_List
     *
     * @param json JSON字符串
     * @return String_List
     * @throws JsonSyntaxException 转换异常,json不合法或者目标对象类型不合法
     */
    public static List<String> toStringList(String json) throws JsonSyntaxException {
        return gson.fromJson(json, TYPE_OF_STRING_LIST);
    }
