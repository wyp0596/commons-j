...
public abstract class FTPUtils {
    ...
    public static void init(String host, int ftpPort, String ftpUsername, String ftpPassword) {
        HOST = host;
        PORT = ftpPort;
     ...
     */
    public static boolean upload(String remoteDir, String remoteFileName, File localFile) {
        FTPClient ftp = null;
        try {
     ...
     * false - 下载失败
     */
    public static boolean download(String remoteDir, String remoteFileName, File downloadFile) {
        FTPClient ftp = null;
        try {
            ftp = initFtpClient(remoteDir);
            if (ftp == null) {
    ...
    private static FTPClient initFtpClient(String remoteDir) throws IOException {
        ...
        boolean loginRet = ftp.login(USERNAME, PASSWORD);
        if (!loginRet) {
    ...
    private static boolean MKDAndCWD(FTPClient ftp, String remoteDir) throws IOException {
        // 切分出所有子文件夹,按顺序
